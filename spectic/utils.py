...
from typing_extensions import (
    ...
    NotRequired,
    Required,
    _TypedDictMeta,  # type: ignore[attr-defined]
    get_args,
    get_origin,
    get_type_hints,
)
...
tuple_types_regex = re.compile(
    ...
    + "|".join(
        [
            *[repr(x) for x in (List, Sequence, Iterable, Iterator, Tuple, Deque)],
            "tuple",
            "list",
            "collections.deque",
        ]
...
def get_type_hints_with_generics_resolved(
    ...
    return {
        n: _substitute_typevars(type_, typevar_map) for n, type_ in type_hints.items()
    }
...
def expand_type_var_in_type_hint(
    ...
    if namespace:
        return {
            name: _substitute_typevars(hint, namespace)
...
def _substitute_typevars(obj: Any, typevar_map: Mapping[Any, Any]) -> Any:
    if params := getattr(obj, "__parameters__", None):
        args = tuple(
            _substitute_typevars(typevar_map.get(p, p), typevar_map) for p in params
        )
        return obj[args]
    ...
    if isinstance(obj, TypeVar):
        ...
        if (type_ := typevar_map.get(obj, None)) is not None and not isinstance(
            type_, TypeVar
        ):
            return type_
