...
from .types.secrets import SecretBytes, SecretStr
from .utils import get_origin_or_inner_type

PathType: TypeAlias = Path | PathLike | str
TypeDecodersSequence: TypeAlias = Sequence[
...
def encode_json(value: Any, serializer: Callable[[Any], Any] | None = None) -> bytes:
    ...
    try:
        return (
            msgspec.json.encode(value, enc_hook=serializer)
            if serializer
            else _msgspec_json_encoder.encode(value)
        )
...
@overload
def decode_json(
    value: str | bytes, type_decoders: TypeDecodersSequence | None, strict: bool = ...
) -> Any: ...
